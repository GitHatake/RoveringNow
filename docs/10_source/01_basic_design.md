...
| 保持期間 | **詳細設計で定める。** 短すぎれば開示命令に応じられず、長すぎれば個人情報保護法上の保有期間の問題を生じる |

> **【要確認】** 通信ログの保存期間を一般に義務づける日本の法令は確認できていない。保持期間は法令上の義務ではなく、開示命令への対応可能性と個人情報の最小限保有の両立点として、自ら定める必要がある。
...
> **【未定義】** アプリ運営の責任主体（個人情報取扱事業者、利用規約の制定者）が定まっていない。上記の合意は、この主体が確定して初めて締結できる。**詳細設計フェーズの開始までに確定すること。**
...
| **退会時のデータ取扱い** | 削除・匿名化の範囲を定めること。**作成したグループが残る場合のオーナー権限の扱い**を含む（第9.4節 休眠状態との関係） |
| カードの拡散リスク | プロフィールカードがスクリーンショットで拡散しうることを、登録時および編集時に明示する |
| 準拠 | 個人情報保護法に準拠する。利用規約とプライバシーポリシーを整備する |

> **【要確定・詳細設計開始前】** 退会時のデータ取扱いと監査ログの保持期間は、技術ではなく**業務ルール**であり、利用規約に直結する。「何を作るか」の側の判断であるため、詳細設計へ先送りせず確定すること。
...
| # | 項目 | 影響範囲 | 期限 |
|---|---|---|---|
| 1 | **アプリ運営の責任主体** | 第12.6節。外部合意・規約制定の前提 | **詳細設計開始前** |
| 2 | **退会時のデータ取扱い** | 第13.2節。利用規約に直結 | **詳細設計開始前** |
| 3 | **監査ログの保持期間** | 第12.2節・第13.2節 | **詳細設計開始前** |
| 4 | 認証の判断基準（運用ガイドライン） | 第12.3節 | 詳細設計フェーズ |
| 5 | 障害時の代替告知手段 | 第13.5節 | 詳細設計フェーズ |
| 6 | RPO / RTO の数値 | 第13.5節 | 詳細設計フェーズ |
